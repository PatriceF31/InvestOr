    ...
    UUPSUpgradeable
{
    ...
    error InactiveAccount();      // inactif depuis > 6 mois
    error NoCashbackAvailable();  // aucun frais accumulés
    ...
    /// @notice Initialise les variables cashback (à appeler une seule fois après upgrade)
    ...
    /// @notice Retourne le cashback disponible pour un wallet
    function previewCashback(address user) external view returns (
        uint256 cashbackAmount,
        uint256 totalFees,
        bool    isEligible
    ) {
        isEligible = (lastActivityAt[user] > 0) &&
                    (block.timestamp - lastActivityAt[user] <= 180 days);

        uint256 currentSlot = _currentSlot();
        for (uint256 i = 0; i < 4; i++) {
            if (currentSlot >= i) {
                totalFees += feesBySlot[user][currentSlot - i];
            }
        }
